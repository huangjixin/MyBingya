...
public class ExcelUtil {
	...
	public static <T> void main(String[] args) {
		...
		try {
			...
			// Map<String, List<Map<String, Object>>> ml =
			// testExcel.readExcel(new FileInputStream(new File("E:/1.xlsx")));
			InputStream is = new FileInputStream(new File("E:/1.xlsx"));
			// 测试 1
			/*
			 * String[] names = new String[] { "name", "api_order_id",
			 * "order_id", "start_time", "money" }; System.out.println("names:"
			 * + names.length); Map<String, List<Map<String, Object>>> ml =
			 * testExcel.readExcel(is, names,1); for (Map.Entry<String,
			 * List<Map<String, Object>>> map : ml.entrySet()) {
			 * 
			 * System.out.println(map.getKey()); List<Map<String, Object>> lm =
			 * map.getValue(); for (int j = 0; j < lm.size(); j++) {
			 * 
			 ...
			 * }
			 * 
			 * Workbook wb=testExcel.writeExcel(ml,null); FileOutputStream
			...
			// 测试 2
			/*
			 * Map<String, List<Object[]>> ml = testExcel.readExcel(is,0); for
			 * (Map.Entry<String, List<Object[]>> map : ml.entrySet()) {
			 * 
			 * String key = map.getKey(); System.out.println("key:" + key);
			 * List<Object[]> lo = map.getValue(); for (int j = 0; j <
			 * lo.size(); j++) {
			 * 
			 * Object[] obj = lo.get(j); for (int x = 0; x < obj.length; x++) {
			 * 
			...
			// 测试 3
			/*
			 * List<TbsUserBean> lt=new ArrayList<TbsUserBean>(); TbsUserBean
			 * bean=new TbsUserBean(); bean.setId("111111111");
			 * bean.setCount(100); bean.setIp("127.0.0.1");
			 * bean.setFailCount(0); bean.setPassword("123456");
			 * bean.setUsername("admin"); lt.add(bean); TbsUserBean bean2=new
			 * TbsUserBean(); bean2.setId("22222222"); bean2.setCount(1002);
			 * bean2.setIp("127.0.0.1"); bean2.setFailCount(2);
			 * bean2.setPassword("1234562"); bean2.setUsername("admin2");
			 * lt.add(bean2); Map<String,List<TbsUserBean>> map=new
			 * HashMap<String, List<TbsUserBean>>(); map.put("sheet", lt);
			 * Workbook wb=testExcel.writeExcel2(map, null,1); FileOutputStream
			 * fileOut = new FileOutputStream("E:/workbookT.xls");
			 * wb.write(fileOut); fileOut.close();
			 */
	 ...
	 * @return
	 */
	public <T> Workbook writeExcel2(Map<String, List<T>> ml, String suffix,
			int lineIndex) {
	 ...
	 * @return
	 */
	public Workbook writeExcel(Map<String, List<Map<String, Object>>> ml,
			String suffix, int lineIndex) {
				...
				for (int j = readline; j < rowNum; j++) {
					...
					for (int x = 0; x < cessNum; x++) {
						...
						case Cell.CELL_TYPE_STRING:
							values[x] = cell.getRichStringCellValue()
									.getString();
							break;
	 ...
	 * @return
	 */
	public Map<String, List<Map<String, Object>>> readExcel(InputStream is,
			String[] names, int readline) {
			...
			for (int i = 0; i < Sheets; i++) {
				...
				for (int j = readline; j < rowNum; j++) {
					...
					for (int x = 0; x < cessNum; x++) {
						...
						case Cell.CELL_TYPE_STRING:
							map.put(names[x], cell.getRichStringCellValue()
									.getString());
							break;
						case Cell.CELL_TYPE_NUMERIC:
							if (DateUtil.isCellDateFormatted(cell)) {
	 ...
	 * @return
	 */
	public <T> Map<String, List<T>> readExcel(InputStream is, String[] names,
			int readline, Class<T> bean) {
			...
			for (int i = 0; i < Sheets; i++) {
				...
				for (int j = readline; j < rowNum; j++) {
					...
					for (int x = 0; x < cessNum; x++) {
						...
						Cell cell = row.getCell(x);
						PropertyDescriptor propertyDes = new PropertyDescriptor(
								names[x], t.getClass());
						...
						case Cell.CELL_TYPE_STRING:
							method.invoke(t, cell.getRichStringCellValue()
									.getString());
							break;
						case Cell.CELL_TYPE_NUMERIC:
							if (DateUtil.isCellDateFormatted(cell)) {
