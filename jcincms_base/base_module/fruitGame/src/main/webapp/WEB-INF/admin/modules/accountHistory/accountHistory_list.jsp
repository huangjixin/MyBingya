...
<script type="text/javascript">
	...
	function deleteRows(selecedRow) {
		...
		$.ajax({
			cache : true,
			type : "POST",
			url : '${ctxAdmin}/accountHistory/deleteById',
			data : pamameter,
			async : false,
	...
	function create(){
		window.location.href='${ctxAdmin}/accountHistory/create'; 
	}
	...
	function update(){
		...
		window.location.href='${ctxAdmin}/accountHistory/update/'+row[0].id+''; 
	}
	...
	function show(){
		...
		window.location.href='${ctxAdmin}/accountHistory/show/'+row[0].id; 
	}
	...
	function search() {
		...
		if ($("totalInput").val() != "") {
			queryParams.total = $("#totalInput").val();
		}
		...
		if ($("tbUser_idInput").val() != "") {
			queryParams.tbUser_id = $("#tbUser_idInput").val();
		}
	...
	function exportExcel() {
		window.open('${ctxAdmin}/accountHistory/exportExcel');
	}
...
<body class="easyui-layout" data-options="fit:true">
	...
	<div data-options="region:'center'" title=""
		...
		<div id="toolBar" style="padding: 5px;border: 0px;">
			<shiro:hasPermission name="accountHistory:create">
				<input type="button" value="添加" id="btn_Add" name="btn_Add" onclick="create();" /> 
			</shiro:hasPermission>
			<shiro:hasPermission name="accountHistory:delete">
				<input type="button" value="删除" onclick="deleteRows();" />
			</shiro:hasPermission>
			<shiro:hasPermission name="accountHistory:update">
				<input type="button" value="更新" onclick="update();" /> 
			</shiro:hasPermission>
			<shiro:hasPermission name="accountHistory:view">
		...
		<div style="padding: 5px;border: 0px;">
			...
			<label>total:</label>
			<input  id="totalInput" onkeydown="onKeyEnter(event.keyCode||event.which);">&nbsp;&nbsp;
			...
			<label>tbUser_id:</label>
			<input  id="tbUser_idInput" onkeydown="onKeyEnter(event.keyCode||event.which);">&nbsp;&nbsp;
		...
		<table id="tgrid" title="" class="easyui-datagrid"
			...
			data-options="
								...
								striped : true,
								collapsible : true,
								url: '${ctxAdmin}/accountHistory/select',
								loadMsg : '数据装载中......',
								method: 'get',
								singleSelect : false,
								selectOnCheck: true,
			...
			<thead>
				<tr>
					...
					<th data-options="field:'tbUser_id',align:'center'" width="100%">tbUser_id</th>
