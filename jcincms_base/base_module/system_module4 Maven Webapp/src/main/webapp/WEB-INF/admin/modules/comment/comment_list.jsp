...
<meta http-equiv="keywords" content="Comment列表">
...
<script type="text/javascript">
	...
	function deleteRows(selecedRow) {
		...
		$.ajax({
			cache : true,
			type : "POST",
			url : '${ctxAdmin}/comment/deleteById',
			data : pamameter,
			async : false,
	...
	function create(){
		window.location.href='${ctxAdmin}/comment/create'; 
	}
	...
	function update(){
		...
		window.location.href='${ctxAdmin}/comment/update/'+row[0].id+''; 
	}
	...
	function show(){
		...
		window.location.href='${ctxAdmin}/comment/show/'+row[0].id; 
	}
	...
	function search() {
		...
		if ($("documentIdInput").val() != "") {
			queryParams.documentId = $("#documentIdInput").val();
		}
		...
		if ($("hiddenInput").val() != "") {
			queryParams.hidden = $("#hiddenInput").val();
		}
	...
	function exportExcel() {
		window.open('${ctxAdmin}/comment/exportExcel');
	}
	...
	<div data-options="region:'center'" title=""
		...
		<div id="toolBar" style="padding: 5px;border: 0px;">
			<input type="button" value="添加" id="btn_Add" name="btn_Add"
				onclick="javascript:window.location.href='${ctxAdmin}/comment/create'" /> 
			<input type="button"
		...
		<div style="padding: 5px;border: 0px;">
			...
			<input  id="documentIdInput" onkeydown="onKeyEnter(event.keyCode||event.which);">&nbsp;&nbsp;
			...
			<label>hidden:</label>
			<input  id="hiddenInput" onkeydown="onKeyEnter(event.keyCode||event.which);">&nbsp;&nbsp;
		</div>
		<table id="tgrid" title="" class="easyui-datagrid"
			...
			<thead>
				<tr>
					...
					<th data-options="field:'documentId',align:'center'" width="100%">documentId</th>
					...
					<th data-options="field:'hidden',align:'center'" width="100%">hidden</th>
				</tr>
			</thead>
		</table>
	</div>
