...
<script type="text/javascript">
	...
	function createDataGrid(){
		$('#tgrid').datagrid({
			...
			striped : true,
			collapsible : true,
			url: '${ctxAdmin}/orderComment/select',
			loadMsg : '数据装载中......',
			method: 'get',
			singleSelect : false,
			selectOnCheck: true,
	...
	function deleteRows(selecedRow) {
		...
		$.ajax({
			cache : true,
			type : "POST",
			url : '${ctxAdmin}/orderComment/deleteById',
			data : pamameter,
			async : false,
	...
	function create(){
		window.location.href='${ctxAdmin}/orderComment/create'; 
	}
	...
	function update(){
		...
		window.location.href='${ctxAdmin}/orderComment/update/'+row[0].id+''; 
	}
	...
	function show(){
		...
		window.location.href='${ctxAdmin}/orderComment/show/'+row[0].id; 
	}
	...
	function search() {
		...
		if ($("orderIdInput").val() != "") {
			queryParams.orderId = $("#orderIdInput").val();
		}
	...
	function exportExcel() {
		window.open('${ctxAdmin}/orderComment/exportExcel');
	}
...
<body class="easyui-layout" data-options="fit:true">
	...
	<div data-options="region:'center'" title=""
		...
		<div id="toolBar" style="padding: 5px;border: 0px;">
			<shiro:hasPermission name="orderComment:create">
				<input type="button" value="添加" id="btn_Add" name="btn_Add" onclick="create();" /> 
			</shiro:hasPermission>
			<shiro:hasPermission name="orderComment:delete">
				<input type="button" value="删除" onclick="deleteRows();" />
			</shiro:hasPermission>
			<shiro:hasPermission name="orderComment:update">
				<input type="button" value="查看" onclick="update();" /> 
			</shiro:hasPermission>
			<!--<shiro:hasPermission name="orderComment:view">
		...
		<div style="padding: 5px;border: 0px;">
			...
			<label>orderId:</label>
			<input  id="orderIdInput" onkeydown="onKeyEnter(event.keyCode||event.which);">&nbsp;&nbsp;
		</div>
		<table id="tgrid">
		</table>
	</div>
