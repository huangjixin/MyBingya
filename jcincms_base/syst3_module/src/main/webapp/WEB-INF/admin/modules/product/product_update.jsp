...
<c:set var="imagesLength" value="${fn:length(product.images)}"></c:set>
		...
		$
				.ajaxFileUpload({
					...
					success : function(data, status) {
						...
						var insertText = '<div style="padding:10px 0;" id="imagesDiv'+imagesCount+'"><input name="images['+imagesCount+'].site" value="'+data.fileAddr+'" type="hidden"><img id="titleImg1" src="${ctx}/'+data.fileAddr+'" style="width:160px;height:160px;background-color: gray;border: 1px solid black;" /></div><input type="button" value="删除" onclick="$(\'#imagesDiv'
								+ imagesCount
								+ '\').remove();$(\'#image'
								+ imagesCount + '\').remove();"/>';
...
<body>
	<form id="validForm" action="${ctxAdmin}/product/update/${product.id}" method="post">
		<input name="id" value="${product.id}" type="hidden" /> <input
			id="proBrandId" name="proBrandId" value="${product.proBrandId}"
		...
		<div class="desc">
			<b>商品信息修改</b>&nbsp;&nbsp;<b style="color: red;" id="msg">${msg}</b>
		</div>
		...
		<table class="table">
			<tr>
				...
				<td><input type="text" id="zhName" name="brandName" class="input"
					readonly="readonly" value="${product.brandName}"/> &nbsp; <input type="button"
