...
<html>
<head>
<title>voteChoice管理</title>

<meta http-equiv="pragma" content="no-cache">
<meta http-equiv="cache-control" content="no-cache">
<meta http-equiv="expires" content="0">
<meta http-equiv="keywords" content="VoteChoice列表">
...
<script type="text/javascript">
	...
	function deleteRows(selecedRow) {
		...
		$.ajax({
			cache : true,
			type : "POST",
			url : '${ctxAdmin}/voteChoice/deleteById',
			data : pamameter,
			async : false,
	...
	function create(){
		window.location.href='${ctxAdmin}/voteChoice/create'; 
	}
	...
	function update(){
		...
		window.location.href='${ctxAdmin}/voteChoice/update/'+row[0].id+''; 
	}
	...
	function show(){
		...
		window.location.href='${ctxAdmin}/voteChoice/show/'+row[0].id; 
	}
	...
	function search() {
		...
		if ($("voteCountInput").val() != "") {
			queryParams.voteCount = $("#voteCountInput").val();
		}
		...
		if ($("imageInput").val() != "") {
			queryParams.image = $("#imageInput").val();
		}
		if ($("tbVote_topic_idInput").val() != "") {
			queryParams.tbVote_topic_id = $("#tbVote_topic_idInput").val();
		}
	...
	function exportExcel() {
		window.open('${ctxAdmin}/voteChoice/exportExcel');
	}
...
<body class="easyui-layout" data-options="fit:true">
	...
	<div data-options="region:'center'" title=""
		...
		<div id="toolBar" style="padding: 5px;border: 0px;">
			<shiro:hasPermission name="voteChoice:create">
				<input type="button" value="添加" id="btn_Add" name="btn_Add" onclick="create();" /> 
			</shiro:hasPermission>
			<shiro:hasPermission name="voteChoice:delete">
				<input type="button" value="删除" onclick="deleteRows();" />
			</shiro:hasPermission>
			<shiro:hasPermission name="voteChoice:update">
				<input type="button" value="更新" onclick="update();" /> 
			</shiro:hasPermission>
			<shiro:hasPermission name="voteChoice:view">
		...
		<div style="padding: 5px;border: 0px;">
			...
			<input  id="voteCountInput" onkeydown="onKeyEnter(event.keyCode||event.which);">&nbsp;&nbsp;
			...
			<label>image:</label>
			<input  id="imageInput" onkeydown="onKeyEnter(event.keyCode||event.which);">&nbsp;&nbsp;
			<label>tbVote_topic_id:</label>
			<input  id="tbVote_topic_idInput" onkeydown="onKeyEnter(event.keyCode||event.which);">&nbsp;&nbsp;
		</div>
		<table id="tgrid" title="" class="easyui-datagrid"
			...
			data-options="
								...
								striped : true,
								collapsible : true,
								url: '${ctxAdmin}/voteChoice/select',
								loadMsg : '数据装载中......',
								method: 'get',
								singleSelect : false,
								selectOnCheck: true,
			...
			<thead>
				<tr>
					...
					<th data-options="field:'voteCount',align:'center'" width="100%">voteCount</th>
					...
					<th data-options="field:'image',align:'center'" width="100%">image</th>
					<th data-options="field:'tbVote_topic_id',align:'center'" width="100%">tbVote_topic_id</th>
				</tr>
			</thead>
		</table>
	</div>
