	...
	$('#roleIdInput').combobox({
		url:'<%=basePath%>role/getRole',
							...
							onLoadSuccess : function() { //数据加载完毕事件
								var data = $('#roleIdInput').combobox( 'getData');
								if (data.length > 0) {
									var roleId = ${roleId};
// 									$('#roleIdInput').combobox('select', data[0].name);
									...
									for ( var i = 0; i < data.length; i++) {
										if (roleId == data[i].id) {
											$('#roleIdInput') .combobox('select', data[i].name);
											break;
										}
									}
...
<body>
	<form:form id="validForm" action="${ctx}/userGroup/updateForm"
		method="post" commandName="userGroup">
				...
				<td width="100%">
					<table border="0" cellpadding="3" cellspacing="1" width="100%"
						...
						<tr style="text-align: right; BACKGROUND-COLOR: #F4FAFF; ">
							...
							<th>&nbsp;角色：</th>
							<td nowrap="nowrap" align="left"><input id="roleIdInput" />
								<input type="hidden" id="roleId" name="roleId" />&nbsp; <input type="button"
								value="清除" onclick="javascript:clearRoleIdInput();" /></td>
						</tr>
						<tr style="text-align: right; BACKGROUND-COLOR: #F4FAFF; ">
