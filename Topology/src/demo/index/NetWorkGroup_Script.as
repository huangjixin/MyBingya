...
private var _resids:String;//资源字符串

private var _resourceid:String;//资源id
...
[Bindable]
public function get resids():String
{
	return _resids;
}

public function set resids(value:String):void
{
	_resids = value;
...
{
	/*var xml:XML = new XML(msdataString);
	if(xml){
		var bgXml:XML = xml.dataBox.s.(@n=='background.image')[0];
		if(bgXml){
			bgXml = new XML("<s n='background.image'>bg400</s>");
			delete xml.dataBox.s.(@n=='background.image')[0];
			xml.dataBox.appendChild(bgXml);
		}
	}*/
	box.alarmBox.clear();
	box.clear();
//	xmlSerializer.deserialize(xml.toString());
