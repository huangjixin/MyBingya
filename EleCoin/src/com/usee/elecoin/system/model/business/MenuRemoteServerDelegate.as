...
package com.usee.elecoin.system.model.business
{
	 ...
	 **** 大厅展示远程委托类，用于远程交互调用  **
	 *******************************************/
	public class MenuRemoteServerDelegate
	{
		...
		// 构造函数，初始化相关工作可以放在里面
		//_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/
		public function MenuRemoteServerDelegate(pResponder : IResponder)
		{
			_service = new RemoteObject();  
			_service.endpoint = Global.getInstance().endPoint;  // 端点；
			_service.destination = "menuServiceImpl";        // 目标对象；
		...
		{
			var call:Object = _service.insert( menu);  
			call.addResponder(_responder);  
		}
		...
		{
			var call:Object = _service.update( menu);  
			call.addResponder(_responder);  
		}
		...
		/**
		 * 根据角色id获取菜单，但传入null值的时候，返回全部菜单。 
		 * @param roleId
		 * 
		 */
		public function serializMenuToXml(roleId:String = null):void  
		{  
			var call:Object = _service.serializMenuToXml(roleId);  
			call.addResponder(_responder);  
		} 
